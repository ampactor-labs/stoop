...
Anything can go in unsigned, which zines always allowed. At **the desk**, whoever holds the chair this cycle reads the tray, cuts what will not run, writes the editor's note, and rings the bell. A cut is not a deletion: the piece stays with its maker and can run next cycle. Publishing archives the issue whole and passes the chair round the roster. The bell can go in anybody's calendar as a file with a reminder the day before, and the shelf says when this browser was last backed up, because until then it is the only copy.
