...
- `src/js/NN-name.js`: one file per concern, in load order — `01-store` (the roster, state, migration, and `SEED_ID`), `02-photos` and `02a-dither` (the photo store, the greyscale originals kept on this device, screening to two tones as grain, dots or hard contrast, and the strip that glues one to a page), `03-render` (the roster and the scraps), `04-impose` (formats and the imposition solver), `05-press` (the sheet, the fit meter, scaling to fit, and what the draft on screen actually is), `05a-paste`/`05b-elrender`/`05c-pasteedit`/`05d-inspector`/`05e-drop`/`05f-fit`/`05g-stamps`/`05h-gen` (the paste-up: its model and undo stack, one renderer shared by the screen and the archive, the drag-resize-rotate gestures, the controls that are not geometry, what lands on the sheet by drag or paste, the fit meter, the stamps as one geometry drawn as SVG and as PDF, and the copier generation), `06-desk` and `06a-pour` (pieces, the tray, the cycle, publishing, and pouring the tray onto the pages), `07-shelf` (the archive, the reading view, reprints, and the guard that stops a blank sheet reaching paper), `07a-landing` (an issue file opens as the issue, chrome off, and the press it rode in offers itself at the end), `07b-pages` (the pages as made, read-only, shared by the file, the shelf and the printed sheet), `08-qr` and `09-qr-draw` (the encoder for the back cover, split only to stay under the ceiling), `10-pdf`, `10a-pdf-ttf`, `11-pdf-sheet`, `11a-pdf-paste`, `11b-pdf-flyer` and `11c-pdf-stamps` (a PDF written by hand: the container, the page's own typeface parsed and embedded as a TrueType subset, the imposed sheet, the paste-up drawn over it with the same face and metrics the screen used, the noticeboard flyer, and the stamps and toner), `12-export` and `12a-site` (the self-carrying issue, piece bundles, and the site folder the back covers point at, zipped), `13-sync` (backup export, merge, import), `14-boot` and `14a-bar` (the drawers, events, start; the zine's name edited in the bar). `build.sh` concatenates them inside a single IIFE, so they share one scope with no module plumbing: parts on disk, one function at rest. A new concern is a new file. The build drops whole-line `//` comments from the shipped script and every `/* */` comment from the stylesheet: they are for whoever works on the press, and every issue file would otherwise carry them to every reader. Write an explanation on its own line, never trailing code, and it stays in the source and out of the file. `src/fonts/` holds the two faces the press carries, Anton for headlines and Knewave for the marker, both OFL, subset and wrapped as WOFF with the commands in its README; `build.sh` inlines them as data URLs, and the PDF writer reads them back out of the page's own stylesheet and unpacks them, so there is one copy of each face and it is the one every screen and every sheet uses.
...
Fifteen suites: `01-features` (names, photos, sync), `02-robustness` (corrupt storage, old backups, missing blobs, a phone-sized window), `03-publication` (the desk, the bell, two issues coexisting, format re-flow, the fit meter), `04-selfcarry` (an exported issue opened on a machine with no storage of its own, which then makes the next issue), `05-qr` (the back cover's symbol, compared module for module against `test/qr-fixtures.json`), `06-pdf` (the written PDF, parsed back as bytes), `07-scene` (a scene of four, which is where the two-person assumptions showed), `08-weight` (publishes an issue and weighs it, which is where the size ceilings are actually held), `09-pasteup` (dragging, turning, typing, undo, and whether a collage survives a change of format and reaches the paper), `10-typeface` (both fonts in the PDF are the fonts in the page table for table, the screen's ruler agrees with each PDF width table, each first baseline sits where the browser puts it, and № reaches the paper as №), and `11-edges` (the bugs people walk into without trying: bylines on a scene of one or three, pulling scraps twice, pieces that did not fit at the bell, a format change and the back cover, keys typed into a text box, undo, a transparent cut-out, and a friend's zine opened on a machine that runs its own), and `12-loop` (an issue handed on, a piece made from the file under the maker's own name, sent back, and taken in by opening it; an unsigned scrap, the bell as a calendar event, and the shelf saying when the browser was last backed up), and `13-pages` (the file opens on the pages as made, a cutting across the gutter prints on both pages, and back issues ride with their covers only), and `14-site` (the site zip, read back entry by entry with node's own zlib, and its front door opened from the unzipped folder), and `15-craft` (lightening and re-screening a photograph from the original kept on the device, which never rides in a file; a long piece running on to the next page with every word arriving; the printer-margin guide following the fold; duplicating, moving and dragging cuttings across the gutter without the view jumping; a photograph's description reaching screen readers and the text view).
